...
public class Juego {
    ...
    public ResultadoMovimiento realizar_movimiento(String origen, String destino) {
        ...
        try {
            ...
            return new ResultadoMovimiento(false, "No es correcta: " + e.getMessage());
        }
    ...
    private String ejecutar_movimiento(int fila_o, int col_o, int fila_d, int col_d) {
        ...
        if (capturada != null) {
            ...
            // Si la pieza capturada es el Rey, la partida termina
            if (capturada instanceof Rey) {
                tablero.set_pieza(fila_d, col_d, pieza);
                tablero.set_pieza(fila_o, col_o, null);
                pieza.set_se_movio(true);
                String ganador = turno_actual.equals("BLANCO") ? "BLANCAS" : "NEGRAS";
                return "¡Las " + ganador + " ganan! El Rey ha sido capturado.";
            }
